"""
CombinedPlots.py makes combined Grid and Vertical plots.

The grid spacing of these plots is very specific. The
spacing is specified is matplotlib.gridspec
...
# defaults holds the values of all constants needed for these plots.
#  All these values can be updated through keyword arguments
...
_mod_grid = {'hspace':0.05,
             'left':0.05,
             'wspace':0.05},
_cbar_grid = {  'left':0.15+0.05,
                'right':0.8-0.05,
                'bottom':0.2, 'top':0.4},
_vert_grid = {'left':0.1,
              'right':0.75},
...
_vxlabel = 'Xco$_2$ (ppm)',
_vylabel = 'Distance from source (km)',
_vlatlabel = 'Latitude ($^{\circ}$N)',
_vleftlabelpad = 20,
_vrightlabelpad = 20,
_vplot_src_msize = 8,
_vplot_src_marker = '.',
_vplot_src_colour = 'y',
_vplot_title = 'Number of background points: %s',

background_x_step=None,
...
def main(overpass, fname, **kwargs):
    """Creates a combined plot using the values in defaults, updated
    using any arguments passed in kwargs.
    
    Since we call defaults.update(**kwargs), any and all values in
    defaults can be updated. Also, only values in defaults can be
    updated; an exception will be raised if any other keywords
    are specified.
    """
    params = defaults
